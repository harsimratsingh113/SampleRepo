...
```bash
python3 scripts/jira_automation_codex_runner.py --payload-file "$GITHUB_EVENT_PATH"
```
...
Mode defaults to `dry-run`. Labels such as `codex-open-pr` or `codex-push-pr` switch the
automation entrypoint to `push-pr` unless `--mode` is provided.
